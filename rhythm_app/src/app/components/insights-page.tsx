...
import svgPaths from "../../imports/insights-icons-svg-paths";
    ...
    <div className="bg-white rounded-2xl relative">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-4 mb-3">
          <div>
            <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>{heartRateData.resting}</span>
            <span className="text-[11px] text-[#8A8680] ml-1">bpm resting</span>
          </div>
          <div>
            <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>{heartRateData.hrv}</span>
  ...
  return (
    <div className="bg-white rounded-2xl relative">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-4 mb-3">
          <div>
            <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>{sleepData.lastNight.total}</span>
            <span className="text-[11px] text-[#8A8680] ml-1">hours</span>
          </div>
          <div>
            <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>{sleepData.lastNight.score}</span>
  ...
  return (
    <div className="bg-white rounded-2xl relative">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-4 mb-3">
          <div>
            <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>{activityData.steps.toLocaleString()}</span>
            <span className="text-[11px] text-[#8A8680] ml-1">steps</span>
          </div>
          <div>
            <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>{activityData.calories}</span>
  ...
  return (
    <div className="bg-white rounded-2xl relative">
      ...
      <div className="p-4">
        ...
        <div className="mb-3">
          <span className="text-[22px] text-[#2D2A26]" style={{ fontFamily: "'DM Serif Display', serif" }}>+{temperatureData.deviation}°C</span>
...
export function InsightsPage() {
  ...
  const modelInsightCards = useMemo(() => {
    if (!session?.prediction) return insightCards;
    ...
    const first = {
      ...insightCards[0],
      title: `Today: ${stressLabel} stress, ${moodLabel} mood`,
      description:
        `Your latest model run predicts stress as "${stressLabel}" and mood as "${moodLabel}". ` +
        `Use this as a live context layer for today's choices; this card updates after each new Check-in.`,
      category: "Prediction",
      color: "#8B6E5A",
    };
    return [first, ...insightCards.slice(1)];
  }, [session?.prediction]);
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-6">
      ...
      <div className="mb-5">
        ...
        {session?.prediction && (
          <p className="text-[12px] leading-[18px] text-[#8A8680] mt-1">
            Synced from backend model ({session.prediction.stress_model_source}
            {session.prediction.mood_model_source ? ` + ${session.prediction.mood_model_source}` : ""})
          </p>
        )}
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeSection === "insights" ? (
          ...
          >
            {modelInsightCards.map((card, index) => (
